...
class Policy_iteration(ABC):
    ...
    def Policy_iteration(self):
        ...
        while True:
            ...
            self.policy = self.policy_improvement(self.V)
            if all(old_policy[s] == self.policy[s] for s in self.states) or it>5: #change to theta-variable
                break
            it += 1
        print(self.V)
        return self.policy

    def policy_evaluation(self, gamma = 0.6, theta = 0.1):
    ...
    def policy_improvement(self, V:dict):
        for s in self.states:
            Q = {}
            for a in self.actions:
                ...
                Q[a] = addition + self.gamma*V[s_next]
    ...
    def take_action(self, observation: np.ndarray, info: None | dict) -> int:
        ...
        """
        if not self.states:
            self.states = [(x, y) for x in range(len(observation)) for y in range(len(observation[0]))]
            self.policy = {s: np.random.choice(self.actions) for s in self.states}
            self.obs = observation
            self.rewards = {((x, y), a): 0 for x in range(len(observation)) for y in range(len(observation[0])) for a in self.actions}
            self.V = {s: 0 if (0 < s[0] < len(self.obs)) & (0 < s[1] < len(self.obs[0])) else -5 for s in self.states}
        policy_to_choose = self.Policy_iteration()
